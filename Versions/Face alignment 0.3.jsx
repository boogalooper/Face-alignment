...
const rotateMode = 0 // 0 - поворот головы выключен, 1 - поворот головы включен
const debugMode = 1 // 0 - выключен, 1 - отрисовывать найденные границы лица и центр позиционирования
...
function getFaceBounds(selectedLayers) {
    ...
    function measureFace(measurerment) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (selectionBoundsDesc != null) measurerment[i] = new AM.measureLayer(layerBoundsDesc, selectionBoundsDesc)
    ...
    function getRelativeBounds(id, measurerment) {
        ...
        switch (measurerment.length) {
            ...
            case 2:
                ...
                this.ratioY = 1
                this.angle = 0
                this.X = e.left + (e.right - e.left) / 2
            ...
            case 3:
                ...
                this.ratioX = (r.right - l.left) / (Math.sqrt(Math.pow(r.right - l.left, 2) + Math.pow(r.top - l.top, 2)))
                this.ratioY = 1 + (1 - this.ratioX)
...
function transformLayers(selectedLayers, baseLayer) {
        ...
        if (selectedLayers[i] instanceof Object) {
            ...
            if (selectedLayers[i] instanceof Object) {
                var dH = baseLayer.X - selectedLayers[i].X,
                    ...
                    scale = selectedLayers[i].height > selectedLayers[i].width ? scaleH * selectedLayers[i].ratioY : scaleW * selectedLayers[i].ratioX,
    ...
    function drawDebugRect(layer) {
        AM.draw(layer.top *layer.ratioX , layer.left* layer.ratioY, layer.bottom * layer.ratioY, layer.right * layer.ratioX, "00ff00", 16)
        AM.draw(layer.Y - 10, layer.X - 10, layer.Y + 10, layer.X + 10, "00ff00", 16)

        var len = layer.measurerment.length
        for (var i = 0; i < len; i++) {
            cur = layer.measurerment[i]
            AM.draw(cur.top, cur.left, cur.bottom, cur.right, "fff600", 8)
            AM.draw(cur.Y - 10, cur.X - 10, cur.Y + 10, cur.X + 10, "ff0000", 8)
        }
    }
}
